...
public class OperacionesPrerender {
	...
	private StringBuffer getItemsInifniteMagazine(ArrayList<NotaDTO> listNotasMagazineLanding, ParametrosDTO parametrosDTO){
		StringBuffer items=new StringBuffer();
		StringBuffer item=new StringBuffer();
		
		String idContenidosNoRepetidos="";
		try {
			logger.info("listNotasMagazineLanding.size [Tienne que ser 9]: "+listNotasMagazineLanding.size());
			...
			for (int i = 4; i < listNotasMagazineLanding.size(); i++) {
				item=new StringBuffer();
				item.append(" <a href=\""+listNotasMagazineLanding.get(i).getFcLinkDetalle().replaceAll("http://www.unotv.com", "").replace("especialess", "especiales")+"\" class=\"item-note\"> \n");
				...
				item.append(" </a> \n");
				items.append(item);
				logger.info("Detalle Nota ----->"+listNotasMagazineLanding.get(i).getFcLinkDetalle());
			}
			...
			logger.info("Split Length,, supuestamente 9: "+idContenidosNoRepetidos.split("\\,").length);
	...
	/*private StringBuffer getItemsInifnite(ArrayList<NotaDTO> listNotasMagazineLanding, ParametrosDTO parametrosDTO){
		StringBuffer items=new StringBuffer();
		StringBuffer item=new StringBuffer();
		int contador=0;
		try {
			LlamadasWS llamadasWS=new LlamadasWS(parametrosDTO.getURL_WS_BASE());
			ArrayList<NotaDTO> listNotasMagazineHome=(ArrayList<NotaDTO>) llamadasWS.getNotesPublished(parametrosDTO.getIdMagazineHome());
			if(listNotasMagazineHome!= null && listNotasMagazineHome.size()>0){
				
				for (int i = 0; i < listNotasMagazineHome.size(); i++) {
					for (int j = 0; j < listNotasMagazineLanding.size(); j++) {
						if (listNotasMagazineHome.get(i).getFcIdContenido().equals(listNotasMagazineLanding.get(j).getFcIdContenido())) {
							listNotasMagazineHome.remove(i);
							contador++;
						}
					}
				}
				for (int i = 0; i < listNotasMagazineHome.size(); i++) {
					item=new StringBuffer();
					item.append(" <a href=\""+listNotasMagazineHome.get(i).getFcLinkDetalle().replaceAll("http://www.unotv.com", "")+"\" class=\"item-note\"> \n");
					item.append(" <div class=\"thumb\"> \n");
					item.append(" <img src=\"utils/img/blank.gif\" data-echo=\""+listNotasMagazineHome.get(i).getFcImgPrincipal()+"\"></div> \n");
					item.append(" <h3> \n");
					item.append(" 	<span class=\"title\"> \n");
					if(listNotasMagazineHome.get(i).getFcIdTipoNota().equalsIgnoreCase("video"))
						...
						item.append(" 		<i class=\"fa fa fa-camera-retro\"></i> \n");
					item.append(" "+ cambiaCaracteres(listNotasMagazineHome.get(i).getFcTitulo())+" \n");
					item.append(" 	</span> \n");
					item.append(" 	<span class=\""+listNotasMagazineHome.get(i).getFcIdCategoria()+" tag\">"+cambiaCaracteres(listNotasMagazineHome.get(i).getFcDescripcionCategoria())+"</span> \n");
					item.append(" </h3> \n");
					item.append(" </a> \n");
					items.append(item);
				}
			...
			try {
				for (int i = 0; i < 4; i++) {
					...
					if(listNoticias.get(i).getFcNombre().toLowerCase().contains("http://") || listNoticias.get(i).getFcNombre().toLowerCase().contains("https://"))
						urlDetalle=listNoticias.get(i).getFcNombre()+"/"+paramEstadoSeleccionado;
					else
						urlDetalle=listNoticias.get(i).getFcLinkDetalle().replace("especialess", "especiales")+"/"+paramEstadoSeleccionado;
					
					logger.info("Titulo ----->"+listNoticias.get(i).getFcTitulo());
				...
				sbInifnite.append(getItemsInifniteMagazine(listNoticias, parametrosDTO));
